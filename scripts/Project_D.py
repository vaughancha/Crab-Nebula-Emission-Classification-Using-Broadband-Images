...
#calibrated FITS and PNGs go into the git repo folders
out_data_dir = "/home/connor/git/Crab-Nebula-Emission-Classification-Using-Broadband-Images/data"
...
for band, img in imgs_cal.items():
    ...
    out = f"{out_data_dir}/Crab_{band}_cal.fits"
...
plt.tight_layout()
plt.savefig(f"{out_dir}/calibration_check.png", dpi=120, bbox_inches='tight')
plt.show()
...
B = fits.getdata(f"{out_data_dir}/Crab_B_cal.fits").astype(float)
V = fits.getdata(f"{out_data_dir}/Crab_V_cal.fits").astype(float)
R = fits.getdata(f"{out_data_dir}/Crab_R_cal.fits").astype(float)
...
plt.suptitle('Crab Nebula — ratio maps  (sky-subtracted, 3σ+star mask, log scale)',
             fontsize=12)
plt.tight_layout()
plt.savefig(f"{out_dir}/ratio_maps.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.suptitle('Crab Nebula — color-color diagram  (each point = 1 smoothed pixel)',
             fontsize=12)
plt.tight_layout()
plt.savefig(f"{out_dir}/color_color.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(f"{out_dir}/spatial_overlay.png", dpi=150, bbox_inches='tight')
plt.show()
